...
public:
	...
	pair<bool, int> lower_bound(int target){
	    if(root){
            BinaryNode* res = _lower_bound(target, root);
    ...
    BinaryNode* _lower_bound(int target, BinaryNode* root){
        if(!root){
            return nullptr;
        }
        if(target <= root->data){
            BinaryNode* ans = _lower_bound(target, root->left);

            if(ans) return ans;

            return root;
        }
        return _lower_bound(target, root->right);
    }
};
...
void test1(){
   ...
   int arr[] = {9,15,17,20,22,25,27};

    for (int i = 0; i < 7; ++i) {
		tree.insert_value(arr[i]);
	}
	pair<bool,int>p = tree.lower_bound(24);
